...
- Garage Admin API + S3 API を Web UI で操作可能 (Workers は一覧のみ、操作系は対象外)
...
| Backend | Go 1.26, garage-admin-sdk-golang v2.1.0 (`git.deuxfleurs.fr/garage-sdk/garage-admin-sdk-golang`), aws-sdk-go-v2 |
...
Object key はクエリパラメータで受け渡す。Frontend で `encodeURIComponent(key)` してクエリに載せ、Backend は `r.URL.Query().Get("key")` で取得する (Go の標準ライブラリが自動デコードするため、明示的な `url.QueryUnescape` は不要)。
...
- Mutation リクエストに `X-Requested-With: XMLHttpRequest` ヘッダーを要求 (HTML form からの送信を防止)
- JSON ボディのエンドポイントは `Content-Type: application/json` を検証
- ファイルアップロード (`POST /api/objects/:bucket/upload`) は `multipart/form-data` を許可し、`X-Requested-With` ヘッダーのみで CSRF 保護
